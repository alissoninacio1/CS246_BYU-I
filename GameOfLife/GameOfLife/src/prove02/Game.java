...
public class Game {
    ...
    */
	public void run()
	{
		...
		// add 10 Zombie instances to the List of Creature objects
		for (int i = 0; i < 10; i++) {
			Zombie z = new Zombie();
			creatures.add(z);
		}

		// add 10 Wolves instances to the List of Creature objects
		for (int i = 0; i < 10; i++) {
			Wolf w = new Wolf();
			creatures.add(w);
		}

		//add 15 Mutants instances to the list of Creatures objects
		for(int i = 0; i < 15; i++) {
			Mutant m = new Mutant();
			creatures.add(m);
		}
