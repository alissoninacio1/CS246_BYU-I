...
public class CreatureHandler
{
	...
	*/
	public void updateCreatures() {
		/**
		 * Modify the CreatureHandler class so that it handles the Spawner behavior.
		 * It should handle this behavior in a similar way to how it handles the other behaviors.
		 * Spawner behaviors should be handled after Aggressor behaviors.
		 *
		 * If we hold temporarily these new creatures(children), we could hold them in an array and loop through it
		 *
		 * To add an object to the ArrayList, we call the add() method on the ArrayList,
		 * passing a pointer to the object we want to store.
		 */
		...
		for(Creature c : _creatures) {
				...
				if (c instanceof Spawner) {
					...
					Creature cNew = s.newCreatureSpawned();
					if (null == cNew) {
						continue;
					}
					newCreatures.add(cNew);
				}
			}
		/**
		 * The addAll() is a method of Java Collections class
		 * which adds all specified elements to the specified collection.
